...
class AdminMissionsPage extends Page implements HasTable
{
    ...
    private const AUDIT_FIELDS = ['title', 'description', 'type', 'game_id', 'target_amount', 'reward', 'image', 'status'];
            ...
            ->headerActions([
                Tables\Actions\Action::make('create_mission')
                    ...
                    ->action(function (array $data): void {
                        ...
                        $mission = Mission::query()->create($this->normalizePayload($data));
                        $this->clearMissionCache();
                        AdminAudit::log('mission.create', $mission, [], $mission->only(self::AUDIT_FIELDS), 'Criou missão administrativa.');

                        Notification::make()->title('Missão criada')->body('A missão foi cadastrada com sucesso.')->success()->send();
                    }),
            ])
            ->columns([
                ...
                Tables\Columns\TextColumn::make('title')->label('Missão')->searchable()->sortable()->weight('bold')
                ...
                Tables\Columns\TextColumn::make('type')->label('Tipo')->badge()
                    ...
                    ->color(fn (?string $state): string => match ($state) {
                        'deposit' => 'success', 'rounds_played' => 'info', 'win_amount' => 'warning', 'loss_amount' => 'danger', 'game_bet', 'total_bet' => 'primary', default => 'gray',
                    })->sortable(),
                Tables\Columns\TextColumn::make('target_amount')->label('Meta')->formatStateUsing(fn ($state, Mission $record): string => $this->formatTarget($record))->sortable(),
                Tables\Columns\TextColumn::make('reward')->label('Recompensa')->money('BRL')->sortable(),
                Tables\Columns\TextColumn::make('game_id')->label('Jogo')->state(fn (Mission $record): string => $this->gameName($record->game_id))->placeholder('Todos')->toggleable(),
                Tables\Columns\TextColumn::make('status')->label('Status')->badge()
                    ->formatStateUsing(fn (?string $state): string => $state === 'active' ? 'Ativa' : 'Inativa')
                    ->color(fn (?string $state): string => $state === 'active' ? 'success' : 'danger'),
                Tables\Columns\TextColumn::make('updated_at')->label('Atualizada')->dateTime('d/m/Y H:i')->sortable()->toggleable(isToggledHiddenByDefault: true),
            ])
            ->filters([
                Tables\Filters\SelectFilter::make('status')->label('Status')->options(['active' => 'Ativas', 'inactive' => 'Inativas']),
                Tables\Filters\SelectFilter::make('type')->label('Tipo')->options($this->missionTypeOptions()),
                Tables\Filters\Filter::make('with_game')->label('Com jogo vinculado')
                    ->query(fn (Builder $query): Builder => $query->whereNotNull('game_id')->where('game_id', '!=', '')),
            ])
            ->actions([
                Tables\Actions\Action::make('preview')->label('Visualizar')->icon('heroicon-o-eye')->color('gray')->action(fn (Mission $record) => $this->openPreview($record)),
                Tables\Actions\Action::make('edit_mission')
                    ...
                    ->fillForm(fn (Mission $record): array => [
                        'title' => $record->title, 'description' => $record->description, 'type' => $record->type,
                        'game_id' => $record->game_id, 'target_amount' => $record->target_amount, 'reward' => $record->reward,
                        'image' => $this->normalizeFileUploadStateForForm($record->image), 'status' => $record->status, 'admin_password' => null,
                    ])
                    ...
                    ->action(function (Mission $record, array $data): void {
                        ...
                        $fresh = $record->fresh();
                        $this->clearMissionCache();
                        AdminAudit::log('mission.update', $fresh, $before, $fresh->only(self::AUDIT_FIELDS), 'Atualizou missão administrativa.');
                        Notification::make()->title('Missão atualizada')->success()->send();
                    }),
                Tables\Actions\Action::make('toggle_status')
                    ...
                    ->action(function (Mission $record, array $data): void {
                        ...
                        $fresh = $record->fresh();
                        $this->clearMissionCache();
                        AdminAudit::log('mission.status.update', $fresh, $before, $fresh->only(self::AUDIT_FIELDS), 'Alterou status da missão.');
                        Notification::make()->title('Status alterado')->success()->send();
                    }),
                Tables\Actions\Action::make('delete_mission')
                    ...
                    ->action(function (Mission $record, array $data): void {
                        ...
                        $before = $record->only(self::AUDIT_FIELDS);
                        $id = $record->getKey();
                        AdminAudit::log('mission.delete', $record, $before, [], 'Excluiu missão administrativa.');
                        $record->delete();
                        $this->clearMissionCache();
                        Notification::make()->title('Missão excluída')->success()->send();
                    }),
            ])
            ->bulkActions([])
    ...
    private function missionFormSchema(bool $isEdit = false): array
    {
        return [
            Section::make('Como o frontend exibe a missão')
                ...
                ->schema([
                    Placeholder::make('vue_label')->label('Etiqueta no card Vue')->content(fn (Get $get): HtmlString => new HtmlString(
                        '<span style="display:inline-flex;padding:6px 10px;border-radius:999px;background:rgba(249, 115, 22,.14);color:#fdba74;font-weight:900;font-size:12px">'.e($this->missionTypeLabel((string) $get('type'))).'</span>'
                    )),
                    Placeholder::make('vue_description')->label('Descrição curta automática')->content(fn (Get $get): string => $this->shortDescriptionForType((string) $get('type'), (float) ($get('target_amount') ?: 0))),
                    Placeholder::make('mission_help')->label('Resumo dos tipos')->content(new HtmlString('
            ...
            Section::make('Informações da missão')->description('Configure o card, objetivo e recompensa da missão.')
                ->schema([
                    TextInput::make('title')->label('Título da missão')->placeholder('Ex.: Deposite R$ 50 e ganhe bônus')->required()->live(onBlur: true)->maxLength(255)->columnSpanFull(),
                    Textarea::make('description')->label('Descrição')->placeholder('Explique de forma simples o que o usuário precisa fazer.')->rows(3)->columnSpanFull(),
                    Select::make('type')->label('Tipo de missão')->options($this->missionTypeOptions())->required()->live()
                        ->afterStateUpdated(function (callable $set, $state): void { if (! in_array($state, $this->typesThatRequireGame(), true)) $set('game_id', null); }),
                    Select::make('status')->label('Status')->options(['active' => 'Ativa', 'inactive' => 'Inativa'])->default('active')->required()->native(false),
                    Select::make('game_id')->label('Jogo vinculado')->options(fn (): array => $this->gameOptions())->searchable()->nullable()
                    ...
                    TextInput::make('target_amount')->label('Meta da missão')->numeric()->inputMode('decimal')->required()->minValue(0.01)->live(onBlur: true)
                        ->helperText(fn (Get $get): string => $this->targetHelper((string) $get('type'))),
                    TextInput::make('reward')->label('Recompensa')->prefix('R$')->numeric()->inputMode('decimal')->required()->minValue(0.01)
                        ->helperText('Valor creditado em bônus quando o usuário resgata a missão.'),
                    FileUpload::make('image')->label('Imagem da missão')->image()->required(! $isEdit)->helperText('Recomenda-se Tamanho 316x220')
                        ->saveUploadedFileUsing(fn (TemporaryUploadedFile $file) => \Helper::upload($file)['path'] ?? null)->columnSpanFull(),
                ])->columns(2),
            Section::make('Confirmação administrativa')->description('Criar ou alterar missão afeta diretamente recompensas e progressão.')
    ...
    public function openPreview(Mission $record): void { $this->previewMission = $record; $this->showPreviewModal = true; }
    public function closePreview(): void { $this->previewMission = null; $this->showPreviewModal = false; }

    public function missionTypeOptions(): array
    {
        return ['deposit' => 'Missão de Depósito', 'game_bet' => 'Missão por Jogo', 'total_bet' => 'Missão de Aposta Total', 'rounds_played' => 'Missão de Rodadas', 'win_amount' => 'Missão de Ganho', 'loss_amount' => 'Missão de Perda'];
    }

    public function missionTypeLabel(?string $type): string
    {
        return match ($type) { 'deposit' => 'DEPÓSITO', 'game_bet' => 'JOGO', 'total_bet' => 'APOSTA TOTAL', 'rounds_played' => 'SPINS', 'win_amount' => 'GANHOS', 'loss_amount' => 'PERDAS', default => 'MISSÃO' };
    }
    ...
    {
        return match ($type) {
            'rounds_played' => 'Informe a quantidade de rodadas/spins necessária.', 'deposit' => 'Informe o valor de depósito aprovado necessário.',
            'win_amount' => 'Informe o valor de ganho necessário.', 'loss_amount' => 'Informe o valor de perda necessário.',
            'game_bet' => 'Informe o valor apostado necessário no jogo vinculado.', 'total_bet' => 'Informe o valor total apostado necessário no dia.',
    ...
    {
        return $record->type === 'rounds_played' ? number_format((float) $record->target_amount, 0, ',', '.') . ' spins' : $this->money($record->target_amount);
    }

    public function gameOptions(): array { return DB::table('games')->orderBy('game_name')->pluck('game_name', 'game_id')->toArray(); }
    public function gameName(?string $gameId): string { return blank($gameId) ? 'Todos' : (string) (DB::table('games')->where('game_id', $gameId)->value('game_name') ?: $gameId); }
    public function typesThatRequireGame(): array { return ['game_bet', 'rounds_played', 'win_amount', 'loss_amount']; }
    ...
    {
        return [
            'total' => Mission::query()->count(), 'active' => Mission::query()->where('status', 'active')->count(),
            ...
            'redeemed_total' => MissionUser::query()->where('redeemed', 1)->count(), 'reward_total' => (float) Mission::query()->sum('reward'),
    ...
    {
        if (method_exists(Mission::class, 'clearMissionCaches')) { Mission::clearMissionCaches(); return; }
