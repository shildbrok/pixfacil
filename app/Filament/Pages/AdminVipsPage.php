...
class AdminVipsPage extends Page implements HasTable
{
        ...
        return $table
            ...
            ->headerActions([
                Tables\Actions\Action::make('create_vip')
                    ->label('Novo nível VIP')
                    ->icon('heroicon-o-plus')
                    ->color('primary')
                    ->slideOver()
                    ->modalWidth('3xl')
                    ->modalHeading('Criar nível VIP')
                    ->form($this->vipFormSchema())
                    ->action(function (array $data): void {
                        Vip::query()->create($this->normalizePayload($data));

                        Notification::make()
                            ->title('Nível VIP criado')
                            ->body('O nível foi cadastrado com sucesso.')
                            ->success()
                            ->send();
                    }),
            ])
            ->columns([
                ImageColumn::make('image')
                    ->label('Imagem')
                    ->getStateUsing(fn (Vip $record): ?string => $this->imageUrl($record->image))
                    ->height(64)
                    ->width(64)
                    ->extraImgAttributes(['style' => 'object-fit: contain; border-radius: 999px; background:#020617;']),
                ...
                Tables\Columns\TextColumn::make('required_missions')
                    ->label('Pontos / missões')
                    ->sortable()
                    ->alignCenter()
                    ->formatStateUsing(fn ($state): string => number_format((int) $state, 0, ',', '.') . ' pts'),

                Tables\Columns\TextColumn::make('weekly_reward')
                    ->label('Bônus semanal')
                    ->money('BRL')
                    ->sortable(),
            ...
            ->filters([
                ...
                Tables\Filters\Filter::make('com_recompensa')
                    ->label('Com recompensa')
                    ->query(fn (Builder $query): Builder => $query->where('weekly_reward', '>', 0)),
            ])
            ->actions([
                Tables\Actions\Action::make('preview')
                    ...
                    ->icon('heroicon-o-eye')
                    ->color('gray')
                    ->action(fn (Vip $record) => $this->openPreview($record)),
    ...
    private function vipFormSchema(bool $isEdit = false): array
    {
        return [
            Section::make('Como a página Vue usa o VIP')
                ...
                ->schema([
                    Placeholder::make('vue_current_preview')
                        ->label('Texto no card Vue')
                    ...
                    Placeholder::make('vue_rules')
                        ->label('Regras do frontend')
            ...
            Section::make('Informações do nível VIP')
                ...
                ->schema([
                    ...
                    TextInput::make('required_missions')
                        ...
                        ->placeholder('Quantidade de missões/resgates necessários')
                        ->numeric()
                        ->inputMode('numeric')
                        ->required()
                        ->minValue(0)
                    ...
                    TextInput::make('weekly_reward')
                        ->label('Bônus semanal')
                        ->prefix('R$')
                        ->placeholder('Valor pago semanalmente')
                        ->numeric()
                        ->inputMode('decimal')
                        ->required()
                        ->minValue(0)
                    ...
                    Textarea::make('description')
                        ->label('Descrição')
                        ->placeholder('Descreva os benefícios deste nível VIP')
                        ->rows(4)
                        ->columnSpanFull(),

                    FileUpload::make('image')
                        ->label('Imagem do nível')
                        ->image()
                        ->required(! $isEdit)
    ...
    public function openPreview(Vip $record): void
    {
        $this->previewVip = $record;
        $this->showPreviewModal = true;
    }

    public function closePreview(): void
    {
        $this->previewVip = null;
    ...
    public function nextVip(?Vip $vip): ?Vip
    {
        ...
        return Vip::query()
            ->where('required_missions', '>', (int) $vip->required_missions)
            ->orderBy('required_missions')
            ->first();
    }
