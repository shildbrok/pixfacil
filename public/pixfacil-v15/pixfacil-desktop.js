...
function shellMarkup(){
  ...
  return `<aside id="pfd-sidebar"><a class="pfd-brand" href="/"><img src="${esc(logo)}" alt="${esc(CFG.softwareName||'Plataforma')}"></a><nav class="pfd-nav">
    ${navItem('home','/','Início')}${navItem('game','/casino/provider/all/category/all','Jogos')}${navItem('live','/casino/provider/all/category/live','Ao vivo')}${navItem('crown','/retro','Originais')}${navItem('gift','/bonus','Bônus')}${navItem('star','/vip','VIP')}
    <div class="pfd-nav-sep"></div>
    ${navItem('user','/profile/account','Minha conta')}${navItem('history','/profile/transactions','Transações')}${navItem('users','/profile/affiliate','Afiliados')}${navItem('support','/support-center','Suporte')}
  </nav><a class="pfd-responsible" href="/profile/responsible-gaming">${ico('shield')}<span><b>18+ Jogo responsável</b><small>Controle e consciência</small></span></a></aside>
...
function mountShell(){
  if(!DESKTOP()||AUTH()||BLOCKED())return;
...
function loadRuntime(){
  ...
  runtimeLoaded=true;
  const s=document.createElement('script');s.src='/pixfacil/runtime.js?v=20260905-1900';s.defer=true;s.dataset.pfUnifiedRuntime='1';document.head.appendChild(s);
}
function boot(){mountShell();loadRuntime();document.addEventListener('keydown',e=>{if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==='k'&&!AUTH()&&!BLOCKED()){e.preventDefault();document.getElementById('pfd-search-input')?.focus()}});addEventListener('resize',()=>{mountShell();loadRuntime()},{passive:true})}
