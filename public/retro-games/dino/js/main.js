(function () {
    ...
    var IS_MOBILE = true; //window.navigator.userAgent.indexOf('Mobi') > -1;
    ...
    Runner.prototype = {
        ...
        update: function () {
            ...
            if (this.activated) {
                ...
                var currentDistance = this.distanceMeter.getActualDistance(this.distanceRan);
                var coinValue = currentDistance * coinRate;
                var saldoElement = document.getElementById('valormoeda1');
                saldoElement.innerHTML = 'R$ ' + coinValue.toFixed(2);
                var playAchievementSound = this.distanceMeter.update(deltaTime, Math.ceil(this.distanceRan));
                if (coinValue >= metaMultiplier * bet) {
                    document.getElementById('popup').style.display = 'block';
                    var valuecoinElement = document.getElementById('valuecoin');
                    valuecoinElement.value = coinValue;
                }
        ...
        startListening: function () {
            ...
            if (IS_TOUCH_ENABLED) {
                ...
                document.addEventListener(Runner.events.CLICK, this);
        ...
        stopListening: function () {
            ...
            if (IS_TOUCH_ENABLED) {
                ...
                document.removeEventListener(Runner.events.CLICK, this);
                document.removeEventListener(Runner.events.MOUSEDOWN, this);
                document.removeEventListener(Runner.events.MOUSEUP, this);
            }
        },
        onKeyDown: function (e) {
            if (!this.crashed && (Runner.keycodes.JUMP[String(e.keyCode)] || e.type == Runner.events.TOUCHSTART || e.type == Runner.events.CLICK)) {
                if (!this.activated) {
                    this.loadSounds();
                    this.activated = true;
                }
        ...
        onKeyUp: function (e) {
            ...
            var isjumpKey = Runner.keycodes.JUMP[keyCode] || e.type == Runner.events.TOUCHEND || e.type == Runner.events.MOUSEDOWN || e.type == Runner.events.MOUSEDOWN;
            ...
            if (this.isRunning() && isjumpKey) {
                ...
                if (Runner.keycodes.RESTART[keyCode] || (e.type == Runner.events.TOUCHEND && e.target == this.touchController) || (e.type == Runner.events.MOUSEUP && e.target == this.canvas) || (deltaTime >= this.config.GAMEOVER_CLEAR_TIME && Runner.keycodes.JUMP[keyCode])) {
                    this.restart();
                }
        ...
        playSound: function (audioElement) {
            if (!IS_TOUCH_ENABLED && audioElement) {
                audioElement.play();
            }
        }
    };
    ...
    function vibrate(duration) {
        if (IS_TOUCH_ENABLED && 'vibrate' in window.navigator) {
