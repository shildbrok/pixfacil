const formatCurrency = (number, showSymbol) => {
  const options = {
    style: "currency",
    currency: "BRL",
  };
...
window.parent.bA = _bet; //set value bet 
const sessionType = window.parent.sT;
const goal = _bet * _meta_multiplier;
...
class Ghost {
  ...
  setMovementStats(pacman, name, level) {
    const pacmanSpeed = pacman.velocityPerMs + pacman.velocityPerMs * 0.1;
...
class Pacman {
  ...
  setStyleMeasurements(scaledTileSize, spriteFrames) {
    ...
    this.animationTarget.style.height = `${this.measurement}px`;
    this.animationTarget.style.width = `${this.measurement}px`;
    this.animationTarget.style.backgroundSize = `${this.measurement * spriteFrames
      }px`;
...
class GameCoordinator {
  constructor() {
    ...
    this.maxFps = 120;
    this.tileSize = 8;
    this.scale = this.determineScale(1.5);
  ...
  reset() {
    ...
    this.pointsDisplay.innerHTML = "R$ 0,00";
    this.highScoreDisplay.innerHTML = formatCurrency(goal, true);
      ...
      row.forEach((block, columnIndex) => {
        if (block === "o" || block === "O") {
          ...
          const points =
            block === "o"
              ? sessionType == "default"
                ? window.parent.bA * 0.0004
                : 0.05
              : sessionType == "default"
                ? window.parent.bA * 0.002
                : 0.08;
  ...
  createBackButton() {
    if (!document.getElementById("go-home")) {
      ...
      backButton.onclick = function () {
        ...
        // document.getElementById("checkout").removeChild();
      };

      document.getElementById("paused-text").appendChild(backButton);
    }
  }
  ...
  handlePauseKey() {
    if (this.allowPause) {
      ...
      winGame(window.parent.wA)
      this.gameEngine.changePausedState(this.gameEngine.running);
      this.soundManager.play("pause");
      this.pausedText.innerText =
        "Voce ganhou R$" + `${formatCurrency(window.parent.wA)}`;
  ...
  awardPoints(e) {
    ...
    if (this.points >= 2 * window.parent.bA && sessionType == 'default') {
      return;
    }

    this.points += e.detail.points;
    window.parent.wA = this.points;


    if (this.points >= goal && document.getElementById("checkout-button")) {
    ...
    if (this.points >= goal && !document.getElementById("checkout-button")) {
      ...
      checkoutButton.innerHTML = "Encerrar aposta";
      ...
      document.getElementById("checkout").appendChild(checkoutButton);
    }

    this.pointsDisplay.innerText = `R$ ${formatCurrency(this.points)}`;
   ...
   */
  deathSequence() {
    if (document.getElementById("checkout-button")) document.getElementById("checkout-button").style.display = "none";
    this.allowPause = false;
    this.cutscene = true;
    this.soundManager.setCutscene(this.cutscene);
    this.soundManager.stopAmbience();
  ...
  gameOver() {
    ...
    new Timer(() => {
      loseGame(window.parent.wA, _bet)
      this.displayText(
        {
  ...
  createFruit() {
    this.removeTimer({ detail: { timer: this.fruitTimer } });
    this.fruit.showFruit(
      this.fruitPoints[this.level] || window.parent.bA * 0.004
    );
  ...
  displayText(position, amount, duration, width, height) {
    ...
    pointsDiv.style.backgroundImage =
      "url(app/style/graphics/" + `spriteSheets/text/${amount}.svg`;
...
class Pickup {
  ...
  reset() {
    this.animationTarget.style.visibility =
      this.type === "fruit" ? "hidden" : "visible";
  }
