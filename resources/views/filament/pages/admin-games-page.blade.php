<x-filament::page>
    ...
    <div class="gm-wrap">
        <section class="gm-card">
            ...
            <div class="gm-stats">
                <div class="gm-stat"><span>Total</span><strong>{{ $stats['total'] }}</strong><small>Jogos</small></div>
                <div class="gm-stat"><span>Ativos</span><strong>{{ $stats['active'] }}</strong><small>Disponíveis</small></div>
                <div class="gm-stat"><span>Inativos</span><strong>{{ $stats['inactive'] }}</strong><small>Ocultos</small></div>
                <div class="gm-stat"><span>Home</span><strong>{{ $stats['home'] }}</strong><small>show_home</small></div>
                <div class="gm-stat"><span>Destaque</span><strong>{{ $stats['featured'] }}</strong><small>is_featured</small></div>
                <div class="gm-stat"><span>Originais</span><strong>{{ $stats['original'] }}</strong><small>original</small></div>
                <div class="gm-stat"><span>Sem categoria</span><strong>{{ $stats['without_category'] }}</strong><small>Revisar</small></div>
                <div class="gm-stat"><span>Sem capa</span><strong>{{ $stats['without_cover'] }}</strong><small>Visual</small></div>
                <div class="gm-stat"><span>Provedores</span><strong>{{ $stats['providers'] }}</strong><small>providers</small></div>
                <div class="gm-stat"><span>Vínculos</span><strong>{{ $stats['category_links'] }}</strong><small>category_game</small></div>
                <div class="gm-stat"><span>Última edição</span><strong style="font-size:12px">{{ $stats['last_update'] ? \Carbon\Carbon::parse($stats['last_update'])->format('d/m/Y H:i') : '-' }}</strong><small>games.updated_at</small></div>
            </div>
        </section>
    ...
    @if($showPreviewModal && $previewGame)
        <div class="gm-modal-backdrop" wire:click.self="closePreview">
            <div class="gm-modal">
                <div class="gm-modal-head">
                    <div>
                        <h3>{{ $previewGame->game_name }}</h3>
                        <p>{{ $previewGame->provider?->name ?: 'Sem provedor' }}</p>
                    </div>
                ...
                <div class="gm-modal-body">
                    <div class="gm-preview">
                        ...
                        <div class="gm-preview-body">
                            <div class="gm-preview-title">{{ $previewGame->game_name }}</div>
                            <div class="gm-preview-desc">
                                {{ $previewGame->description ?: 'Jogo do catálogo local.' }}
                            </div>
                    ...
                    <div class="gm-info-grid">
                        <div class="gm-info"><span>Game ID</span><strong>{{ $previewGame->game_id ?: '—' }}</strong></div>
                        <div class="gm-info"><span>Game Code</span><strong>{{ $previewGame->game_code ?: '—' }}</strong></div>
                        <div class="gm-info"><span>Distribuição</span><strong>{{ $previewGame->distribution ?: '—' }}</strong></div>
                        <div class="gm-info"><span>Views</span><strong>{{ number_format((int) $previewGame->views, 0, ',', '.') }}</strong></div>
                        <div class="gm-info"><span>RTP</span><strong>{{ $previewGame->rtp !== null ? $previewGame->rtp . '%' : '—' }}</strong></div>
                        <div class="gm-info"><span>Atualizado</span><strong>{{ $previewGame->updated_at?->format('d/m/Y H:i') ?: '—' }}</strong></div>
                    </div>
                </div>
            </div>
        </div>
