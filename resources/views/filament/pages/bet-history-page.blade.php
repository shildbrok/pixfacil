<x-filament::page>
    ...
    <div class="hist-wrap">
        <section class="hist-card">
            <div class="hist-hero">
                <h2 class="hist-title">Histórico de Apostas</h2>
                <p class="hist-sub">Consulte perdas/apostas debitadas, ganhos creditados, rodadas, carteiras e provedores com opção de exclusão.</p>
            </div>
            ...
            <div class="hist-stats">
                ...
                <div class="hist-stat">
                    <span>Perdas / apostas</span>
                    <strong>{{ $stats['bet_count'] }}</strong>
                    <small style="display:block;margin-top:4px;color:#a3a3a3;font-size:11px">R$ {{ number_format($stats['total_bet'], 2, ',', '.') }}</small>
                </div>
                <div class="hist-stat">
                    <span>Ganhos</span>
                    <strong>{{ $stats['win_count'] }}</strong>
                    <small style="display:block;margin-top:4px;color:#a3a3a3;font-size:11px">R$ {{ number_format($stats['total_win'], 2, ',', '.') }}</small>
                </div>
                <div class="hist-stat">
                    <span>Resultado da casa</span>
                    <strong style="font-size:14px">R$ {{ number_format($stats['house_result'], 2, ',', '.') }}</strong>
                </div>
            </div>
        </section>
