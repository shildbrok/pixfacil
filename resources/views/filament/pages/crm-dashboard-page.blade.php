    ...
    <div class="crm-wrap">
        <section class="crm-card">
            ...
            <div class="crm-stats big">
                <div class="crm-stat">
                    ...
                    <strong>{{ $this->money($stats['total_deposits']) }}</strong>
                    <small>{{ $stats['total_deposit_count'] }} depósitos pagos</small>
                </div>

                <div class="crm-stat">
                    <span>Total de saques</span>
                    <strong>{{ $this->money($stats['total_withdrawals']) }}</strong>
                    <small>{{ $stats['total_withdrawal_count'] }} solicitações</small>
                </div>

                <div class="crm-stat">
                    <span>Players jogando agora</span>
                    <strong>{{ $stats['players_now'] }}</strong>
                    <small>Jogadores ativos últimos 5 min</small>
                </div>

                <div class="crm-stat">
                    <span>Total de perdas</span>
                    <strong>{{ $this->money($stats['bets_today']) }}</strong>
                    <small>Apostas hoje</small>
                </div>

                <div class="crm-stat">
                    <span>Total de ganhos</span>
                    <strong>{{ $this->money($stats['wins_today']) }}</strong>
                    <small>Ganhos pagos hoje</small>
                </div>

                <div class="crm-stat">
                    <span>Lucro da casa</span>
                    <strong>{{ $this->money($stats['house_profit_today']) }}</strong>
                    <small>Hoje</small>
                </div>
        ...
        <section class="crm-card">
            ...
            <div class="crm-stats">
                <div class="crm-stat">
                    ...
                    <strong>{{ $this->money($stats['deposits_today']) }}</strong>
                    <small>vs ontem: {{ $stats['deposits_vs_yesterday'] }}</small>
                </div>

                <div class="crm-stat">
                    <span>Saques hoje</span>
                    <strong>{{ $this->money($stats['withdrawals_today']) }}</strong>
                    <small>vs ontem: {{ $stats['withdrawals_vs_yesterday'] }}</small>
                </div>

                <div class="crm-stat">
                    <span>Fluxo líquido hoje</span>
                    <strong>{{ $this->money($stats['net_flow_today']) }}</strong>
                    <small>vs ontem: {{ $stats['net_flow_vs_yesterday'] }}</small>
                </div>

                <div class="crm-stat">
                    <span>Conversão novos</span>
                    <strong>{{ $this->pct($stats['new_conversion']) }}</strong>
                    <small>{{ $stats['new_conversion_label'] }}</small>
                </div>

                <div class="crm-stat">
                    <span>Ticket médio hoje</span>
                    <strong>{{ $this->money($stats['avg_ticket_today']) }}</strong>
                    <small>{{ $stats['deposit_count_today'] }} depósito(s)</small>
                </div>
        ...
        <section class="crm-card">
            ...
            <div class="crm-stats">
                <div class="crm-stat">
                    ...
                    <strong>{{ $this->money($stats['player_balance']) }}</strong>
                    <small>Somatório das carteiras dos usuários</small>
                </div>

                <div class="crm-stat">
                    <span>Saldo PlayFiver</span>
                    <strong>{{ $this->money($stats['playfiver_total']) }}</strong>
                    <small>{{ $stats['playfiver_count'] }} carteiras • Operacional: {{ $this->money($stats['playfiver_operational']) }} • Informativo: {{ $this->money($stats['playfiver_info']) }}</small>
                </div>

                <div class="crm-stat">
                    <span>Carteira Principal PF</span>
                    <strong>{{ $this->money($stats['playfiver_primary']) }}</strong>
                ...
                <div class="crm-bars">
                    @foreach($flow as $row)
                        <div class="crm-bar-row">
                            <div>{{ $row['date'] }}</div>
                            <div class="crm-bar-bg"><div class="crm-bar-fill" style="width: {{ $this->barWidth($row['deposits'], $maxFlow) }}%"></div></div>
        ...
        <section class="crm-card crm-section">
            ...
            <table class="crm-mini-table">
                ...
                <tbody>
                    @foreach($this->providerResume() as $row)
                        <tr>
