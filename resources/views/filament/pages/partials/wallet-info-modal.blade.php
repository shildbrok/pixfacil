<div class="wallet-info-modal">
    ...
    <div class="wi-card">
        <div class="wi-user">
            <div>
                <h3 class="wi-title">{{ $wallet->user?->name ?: 'Usuário sem nome' }}</h3>
                <div class="wi-sub">{{ $wallet->user?->email ?: 'E-mail não informado' }}</div>

                <div class="wi-pills">
                    <span class="wi-pill">CPF: {{ $wallet->user?->cpf ?: '-' }}</span>
                    <span class="wi-pill">Telefone: {{ $wallet->user?->phone ?: '-' }}</span>
                    <span class="wi-pill">Perfil: {{ $wallet->user?->hasRole('admin') ? 'Admin' : 'Usuário' }}</span>
                </div>
            </div>
        ...
        <div class="wi-pills">
            <span class="wi-pill">Cadastro: {{ $wallet->user?->created_at?->format('d/m/Y H:i') ?: '-' }}</span>
            <span class="wi-pill">Carteira atualizada: {{ $wallet->updated_at?->format('d/m/Y H:i') ?: '-' }}</span>
        </div>
    </div>
    ...
    <div class="wi-grid">
        <div class="wi-stat success">
            ...
            <strong>{{ $money($wallet->balance_withdrawal) }}</strong>
