<x-filament::page>
    ...
    <div class="hist-wrap">
        <section class="hist-card">
            <div class="hist-hero">
                <h2 class="hist-title">Histórico de Resgate VIP</h2>
                <p class="hist-sub">Acompanhe somente recompensas VIP resgatadas/pagas e o valor total pago aos usuários.</p>
            </div>

            <div class="hist-stats">
                <div class="hist-stat">
                    <span>Total resgates pagos</span>
                    <strong>{{ $stats['total'] }}</strong>
                </div>
                <div class="hist-stat">
                    <span>Usuários premiados</span>
                    <strong>{{ $stats['users'] }}</strong>
                </div>
                <div class="hist-stat">
                    <span>Total pago VIP</span>
                    <strong style="font-size:15px">R$ {{ number_format($stats['total_paid'], 2, ',', '.') }}</strong>
                </div>
                <div class="hist-stat">
                    <span>Último resgate</span>
                    <strong style="font-size:14px">{{ $stats['latest'] }}</strong>
                </div>
            </div>
        </section>
